<template>
  <div class="card-item bg-transparent border-0" style="width: 18rem">
    <div class="img-placeholder"></div>
    <div class="card-body py-2">
      <h1 class="card-title py-2 fw-bolder">Lorem ipsum dolor sit amet?</h1>
      <p class="text-secondary card-text py-2 text-center fs-6 lh-sm">
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse augue sem,
        molestie non maximus non, fermentum quis urna. In porta condimentum odio non
        varius. Nam id euismod augue, sit amet placerat quam.
      </p>
    </div>
  </div>
</template>
<script>
export default {};
</script>
<style lang="css">
/* Img placeholder */
.img-placeholder {
  width: 100%;
  height: 162px;
  background-color: white;
  border: solid black 1px;
}
</style>
